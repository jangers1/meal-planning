...
export default function PantryMasonry({items, color = 'neutral', loading = false, skeletonCount}: PantryMasonryProps) {
        ...
        <Box ref={setContainerEl} className="pantry-masonry" aria-busy={loading}>
            {loading ? (
                ...
                placed.map((item) => {
                    ...
                    return (
                        <Card key={item.id} variant="soft" color={color} className={`pantry-card ${tileClass}`}
                              sx={{p: 1.5}} style={style}>
                            <CardContent sx={{display: 'flex', flexDirection: 'column', gap: 1, width: '100%'}}>
                                <Typography level="title-lg" noWrap>
                                    {item.title}
                                </Typography>
                                <Box sx={{display: 'flex', alignItems: 'center', justifyContent: 'space-between'}}>
                                    <Chip size="sm" variant="solid" color={color} sx={{fontWeight: 600}}>
                                        Qty: {item.quantity}
                                    </Chip>
